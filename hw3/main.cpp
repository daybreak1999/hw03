...
// void setFunc(Arguments *in, Reply *out);
int gesture(void);
void detection(void);
BufferedSerial pc(USBTX, USBRX);
RPCFunction rpcUI(&gestureUI, "f1");
RPCFunction rpcdetec(&angleDetection, "f2");
// RPCFunction rpcset(&setFunc, "setFunction");
int angle[3] = {30, 50, 70};
int selected = 0;
int func = 0;
int events_times = 0;
...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) {
    ...
    if (func == 1) {
        sprintf(buff, "1");
        printf("Publish message: select angle %d\r\n", angle[selected]);
    }
    if (func == 2) {
        sprintf(buff, "2 event #%d", events_times);
        printf("Publish message: over angle %d times\r\n", events_times);
    }
...
void gestureUI(Arguments *in, Reply *out) {
    ...
    char buffer[200];
    char strings[20];
    sprintf(buffer, "gesture UI function");
    ...
    while (func == 1) {
        ...
        if (change_select) {
            ...
            if (gesture_index == 0) 
                ...
                selected = 2;
            uLCD.printf("selected angle: %d", angle[selected]);
        }
        change_select = 0;
        pre_gest = gesture_index;
    }
}
...
void angleDetection(Arguments *in, Reply *out) {
    ...
    char buffer[200];
    char strings[20];
    sprintf(buffer, "angle detection function");
